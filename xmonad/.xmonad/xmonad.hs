...
myWorkplace :: [String]
myWorkplace = ["term", "edit", "web", "chat", "email", "tmp"] ++ (show <$> [7 .. 9])
...
main = do
  ...
  xmonad $
    ...
    desktopConfig
      ...
      , terminal = myTerminal
      , logHook =
          dynamicLogWithPP xmobarPP {ppOutput = hPutStrLn xmproc, ppTitle = xmobarColor "green" "" . shorten 60} <+>
            do 
              dwD <- io darkWallpaperDir
              wwD <- io warmWallpaperDir
              wallpaperSetter def {wallpapers = WallpaperList $ zip myWorkplace (cycle $ WallpaperDir <$> [dwD, wwD])}
...
customerKeyMaps
 ...
 =
  ...
  , ( (modm .|. shiftMask, xK_a)
    , spawn "sleep 0.2; scrot -s '/home/chenyu/screen/%F_$wx$h.png' -e 'xclip -selection clipboard -target image/png -i $f'")
  ...
  , ((0, xK_Print), spawn "scrot '/home/chenyu/screen/%F_$wx$h.png' -e 'xclip -selection clipboard -target image/png -i $f'")
...
popupConfig = def {font = "xft:Inconsolata Nerd Font Complete:antialias=true", position = Top, height = 48}
